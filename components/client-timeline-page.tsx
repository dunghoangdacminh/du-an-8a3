...
export default function ClientTimelinePage({ events = [] }: ClientTimelinePageProps) {
  return (
    <div className="bg-white dark:bg-slate-900 min-h-screen py-12">
      <div className="container px-4 mx-auto">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-blue-800 dark:text-blue-300">Một Năm Đã Qua</h1>
        ...
        {events.length === 0 ? (
          <div className="text-center p-8 bg-blue-50 dark:bg-slate-800 rounded-lg">
            <p className="text-slate-600 dark:text-slate-300">Chưa có sự kiện nào được thêm vào timeline.</p>
          </div>
        ) : (
              ...
              {events.map((event, index) => (
                <motion.div
                  ...
                  className={`relative flex items-center ${
                    index % 2 === 0 ? "flex-row md:flex-row-reverse" : "flex-row-reverse md:flex-row"
                  } gap-8 md:gap-16`}
                >
                  {/* Timeline dot */}
