...
export default function ClientHomePage({ latestMemories = [], featuredEvents = [] }: ClientHomePageProps) {
		...
		<div className="flex flex-col items-center">
			...
			<section className="w-full min-h-[90vh] flex flex-col items-center justify-center relative overflow-hidden bg-gradient-to-b from-blue-50 to-white dark:from-slate-900 dark:to-slate-800">
				...
				<div className="container px-4 z-10 text-center">
					...
					>
						Cùng nhau nhìn lại một năm học đầy kỷ niệm, những khoảnh khắc đáng nhớ và tình bạn đẹp của chúng ta.
					</motion.p>
				...
				>
					<Button variant="ghost" size="icon" className="rounded-full animate-bounce" onClick={scrollToTimeline}>
			...
			<section ref={timelineRef} className="w-full py-20 bg-white dark:bg-slate-900">
				<div className="container px-4">
					...
					>
						<h2 className="text-3xl md:text-4xl font-bold mb-4 text-blue-800 dark:text-blue-300">Một Năm Đã Qua</h2>
						<p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
							Hành trình của chúng ta qua từng tháng với những kỷ niệm khó quên
						</p>
					</motion.div>
					...
					{featuredEvents.length > 0 ? (
						...
						<div className="text-center p-8 bg-blue-50 dark:bg-slate-800 rounded-lg mb-12">
							<p className="text-slate-600 dark:text-slate-300">Chưa có sự kiện nào trong timeline.</p>
						</div>
					)}
			...
			<section className="w-full py-20 bg-blue-50 dark:bg-slate-800">
				<div className="container px-4">
					<motion.div
						...
						className="text-center mb-16"
					>
						<h2 className="text-3xl md:text-4xl font-bold mb-4 text-blue-800 dark:text-blue-300">Góc Kỷ Niệm</h2>
						<p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
						{Array.from({ length: 4 }).map((_, index) => (
							...
							>
								<Image
									src={`/placeholder.svg?height=300&width=300&text=Ảnh ${index + 1}`}
									alt={`Kỷ niệm ${index + 1}`}
									fill
			...
			<section className="w-full py-20 bg-white dark:bg-slate-900">
				<div className="container px-4">
					<motion.div
						...
						className="text-center mb-16"
					>
						<h2 className="text-3xl md:text-4xl font-bold mb-4 text-blue-800 dark:text-blue-300">Lời Muốn Nói</h2>
						<p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
					...
					{latestMemories.length > 0 ? (
								...
								>
									<div className="flex items-center mb-4">
										<div className="relative w-12 h-12 rounded-full overflow-hidden mr-4">
											<Image
												src={memory.avatar || "/placeholder.svg"}
												alt={memory.name}
												fill
												className="object-cover"
											/>
										</div>
										<h3 className="font-bold text-lg text-slate-900 dark:text-white">{memory.name}</h3>
									</div>
									<p className="text-slate-600 dark:text-slate-300 italic">"{memory.message}"</p>
								</motion.div>
							))}
						</div>
					) : (
						<div className="text-center p-8 bg-blue-50 dark:bg-slate-800 rounded-lg mb-12">
							<p className="text-slate-600 dark:text-slate-300">
								Chưa có lời nhắn nào. Hãy là người đầu tiên để lại lời nhắn!
							</p>
						</div>
					)}
			...
			<section className="w-full py-20 bg-gradient-to-t from-blue-50 to-white dark:from-slate-800 dark:to-slate-900 relative overflow-hidden">
				<div className="container px-4 z-10 relative">
					...
					>
						<h2 className="text-3xl md:text-4xl font-bold mb-6 text-blue-800 dark:text-blue-300">Tạm Biệt Tuổi 14</h2>
						<p className="text-lg md:text-xl mb-8 text-slate-700 dark:text-slate-300">
							Tạm biệt lớp 8, hẹn gặp lại năm lớp 9 – năm cuối cấp! Chúc tất cả các bạn có một kỳ nghỉ hè thật vui vẻ và
							chuẩn bị thật tốt cho năm học mới.
						</p>
						<Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white" asChild>
