...
export default function MemoryBoardPage() {
	...
	const fetchMemories = useCallback(async () => {
		try {
			...
			toast({
				title: "Lỗi",
				description: "Không thể tải dữ liệu lưu bút. Vui lòng thử lại sau.",
				variant: "destructive",
			})
		} finally {
			setIsLoading(false)
	...
	return (
		<div className="bg-white dark:bg-slate-900 min-h-screen py-12">
			<div className="container px-4 mx-auto">
				...
				>
					<h1 className="text-4xl md:text-5xl font-bold mb-4 text-blue-800 dark:text-blue-300">Lời Muốn Nói</h1>
							...
							<CardContent className="pt-6">
								<form onSubmit={handleSubmit} className="space-y-4">
									<div className="space-y-2">
										...
										/>
										{errors.name && <p className="text-red-500 text-sm">Vui lòng nhập tên của bạn</p>}
									</div>
									...
									<div className="space-y-2">
										...
										<Textarea
											...
											disabled={isSubmitting}
										/>
										{errors.message && <p className="text-red-500 text-sm">Vui lòng nhập lời nhắn</p>}
									</div>
									...
									<Button type="submit" className="w-full" disabled={isSubmitting}>
										{isSubmitting ? (
											...
											<>
												<Send className="mr-2 h-4 w-4" /> Gửi lời nhắn
											</>
										)}
									</Button>
								</form>
					...
					>
						{isLoading ? (
							<div className="flex justify-center items-center h-64">
								...
								<span className="ml-2 text-slate-600 dark:text-slate-400">Đang tải lưu bút...</span>
							</div>
						) : memories.length === 0 ? (
							<div className="text-center p-8 bg-blue-50 dark:bg-slate-800 rounded-lg">
								<p className="text-slate-600 dark:text-slate-300">Chưa có lời nhắn nào. Hãy là người đầu tiên!</p>
							</div>
						) : (
												...
												<div className="flex items-start gap-4">
													...
													<div className="flex-1">
														<div className="flex justify-between items-start">
															...
															<span className="text-xs text-slate-500 dark:text-slate-400">{memory.date}</span>
														</div>
														<p className="mt-2 text-slate-600 dark:text-slate-300">{memory.message}</p>
													</div>
												</div>
											</CardContent>
										</Card>
