...
export default async function Home() {
	try {
		...
		// Get latest memories
		const memories = await db.collection("memories").find({}).sort({ _id: -1 }).limit(2).toArray()
		...
		return <ClientHomePage latestMemories={memories} featuredEvents={featuredEvents} />
	} catch (error) {
